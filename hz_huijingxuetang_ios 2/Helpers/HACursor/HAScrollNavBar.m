...
//    self.lineView.backgroundColor = WYColorMain;
//    self.lineView.frame = CGRectMake(0, self.frame.size.height-2, _itemTitleBtnWidth, 2);
//
...
//对item进行布局处理
- (void)layoutButtons{
    self.contentSize = CGSizeMake(self.tmpKeys.count * _itemTitleBtnWidth, 0);
    CGFloat buttonW = _itemTitleBtnWidth;
...
//    }
    ...
    if (!self.lineView)
    {
        self.lineView = [[UIView alloc] init];
        if (self.linecoler) {
            self.lineView.backgroundColor = _linecoler;
        }else{
    ...
    if (!self.isLayoutitems)
    {
        ...
        else
        {
            ...
            self.lineView.frame = CGRectMake(CGRectGetMinX(button.frame), self.frame.size.height-2, _itemTitleBtnWidth, 2);
        }
...
-(void)pageChangeBlock:(PageChange)pageBlock
{
