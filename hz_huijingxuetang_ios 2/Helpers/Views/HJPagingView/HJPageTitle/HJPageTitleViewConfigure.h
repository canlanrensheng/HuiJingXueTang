//
//  HJPageTitleViewConfigure.h
//  HJPagingViewExample
//
//  Created by zhangjinshan on 2017/10/16.
...
typedef enum : NSUInteger {
    /// 下划线样式
    HJIndicatorStyleDefault,
    /// 遮盖样式
    HJIndicatorStyleCover,
    /// 固定样式
    HJIndicatorStyleFixed,
    /// 动态样式（仅在 HJIndicatorScrollStyleDefault 样式下支持）
    HJIndicatorStyleDynamic
} HJIndicatorStyle;

typedef enum : NSUInteger {
    /// 指示器位置跟随内容滚动而改变
    HJIndicatorScrollStyleDefault,
    /// 内容滚动一半时指示器位置改变
    HJIndicatorScrollStyleHalf,
    /// 内容滚动结束时指示器位置改变
    HJIndicatorScrollStyleEnd
} HJIndicatorScrollStyle;

@interface HJPageTitleViewConfigure : NSObject
/** 类方法创建 */
+ (instancetype)pageTitleViewConfigure;

#pragma mark - - HJPageTitleView 属性
/** HJPageTitleView 是否需要弹性效果，默认为 YES */
@property (nonatomic, assign) BOOL needBounces;
/** 是否显示底部分割线，默认为 YES */
@property (nonatomic, assign) BOOL showBottomSeparator;
/* HJPageTitleView 底部分割线颜色，默认为 lightGrayColor */
...
@property (nonatomic, assign) HJIndicatorStyle indicatorStyle;
/** 指示器圆角大小，默认为 0f */
...
/** 指示器滚动位置改变样式，默认为 HJIndicatorScrollStyleDefault */
@property (nonatomic, assign) HJIndicatorScrollStyle indicatorScrollStyle;
