...
//  HJPagingViewExample
//
//  Created by zhangjinshan on 17/4/10.
//  Copyright © 2017年 zhangjinshan. All rights reserved.
//

#import "HJPageTitleView.h"
#import "UIView+HJPagingView.h"
#import "UIButton+HJPagingView.h"
#import "HJPageTitleViewConfigure.h"

#define HJPageTitleViewWidth self.frame.size.width
#define HJPageTitleViewHeight self.frame.size.height

#pragma mark - - - HJPageTitleButton
@interface HJPageTitleButton : UIButton
@end
@implementation HJPageTitleButton
- (void)setHighlighted:(BOOL)highlighted {
    
}
@end

#pragma mark - - - HJPageTitleView
@interface HJPageTitleView ()
/// HJPageTitleViewDelegate
@property (nonatomic, weak) id<HJPageTitleViewDelegate> delegatePageTitleView;
/// HJPageTitleView 配置信息
@property (nonatomic, strong) HJPageTitleViewConfigure *configure;
/// scrollView
...
@implementation HJPageTitleView

- (instancetype)initWithFrame:(CGRect)frame delegate:(id<HJPageTitleViewDelegate>)delegate titleNames:(NSArray *)titleNames configure:(HJPageTitleViewConfigure *)configure {
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:0.77];
        if (delegate == nil) {
            @throw [NSException exceptionWithName:@"HJPagingView" reason:@"HJPageTitleView 初始化方法中的代理必须设置" userInfo:nil];
        }
        self.delegatePageTitleView = delegate;
        if (titleNames == nil) {
            @throw [NSException exceptionWithName:@"HJPagingView" reason:@"HJPageTitleView 初始化方法中的标题数组必须设置" userInfo:nil];
        }
        self.titleArr = titleNames;
        if (configure == nil) {
            @throw [NSException exceptionWithName:@"HJPagingView" reason:@"HJPageTitleView 初始化方法中的配置信息必须设置" userInfo:nil];
        }
...
+ (instancetype)pageTitleViewWithFrame:(CGRect)frame delegate:(id<HJPageTitleViewDelegate>)delegate titleNames:(NSArray *)titleNames configure:(HJPageTitleViewConfigure *)configure {
...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
        ...
        _scrollView.frame = CGRectMake(0, 0, HJPageTitleViewWidth, HJPageTitleViewHeight);
...
- (UIView *)indicatorView {
    if (!_indicatorView) {
        ...
        if (self.configure.indicatorStyle == HJIndicatorStyleCover) {
            ...
            if (self.configure.indicatorHeight > self.HJ_height) {
                _indicatorView.HJ_y = 0;
                _indicatorView.HJ_height = self.HJ_height;
            } else if (self.configure.indicatorHeight < tempIndicatorViewH) {
                _indicatorView.HJ_y = 0.5 * (self.HJ_height - tempIndicatorViewH);
                _indicatorView.HJ_height = tempIndicatorViewH;
            } else {
                _indicatorView.HJ_y = 0.5 * (self.HJ_height - self.configure.indicatorHeight);
                _indicatorView.HJ_height = self.configure.indicatorHeight;
            }
            ...
            _indicatorView.HJ_height = indicatorViewH;
            _indicatorView.HJ_y = self.HJ_height - indicatorViewH - self.configure.indicatorToBottomDistance;
        }
        // 圆角处理
        if (self.configure.indicatorCornerRadius > 0.5 * _indicatorView.HJ_height) {
            _indicatorView.layer.cornerRadius = 0.5 * _indicatorView.HJ_height;
        } else {
...
- (UIView *)bottomSeparator {
    if (!_bottomSeparator) {
        ...
        CGFloat bottomSeparatorW = self.HJ_width;
        CGFloat bottomSeparatorH = 0.5;
        CGFloat bottomSeparatorX = 0;
        CGFloat bottomSeparatorY = self.HJ_height - bottomSeparatorH;
...
- (void)setupTitleButtons {
    ...
    if (self.allBtnWidth <= self.bounds.size.width) { // HJPageTitleView 静止样式
        CGFloat btnY = 0;
        CGFloat btnW = HJPageTitleViewWidth / titleCount;
        CGFloat btnH = 0;
        if (self.configure.indicatorStyle == HJIndicatorStyleDefault) {
            btnH = HJPageTitleViewHeight - self.configure.indicatorHeight;
        } else {
            btnH = HJPageTitleViewHeight;
        }
        CGFloat VSeparatorW = 1;
        CGFloat VSeparatorH = HJPageTitleViewHeight - self.configure.verticalSeparatorReduceHeight;
        if (VSeparatorH <= 0) {
            VSeparatorH = HJPageTitleViewHeight;
        }
        CGFloat VSeparatorY = 0.5 * (HJPageTitleViewHeight - VSeparatorH);
        for (NSInteger index = 0; index < titleCount; index++) {
            // 1、添加按钮
            HJPageTitleButton *btn = [[HJPageTitleButton alloc] init];
        ...
        self.scrollView.contentSize = CGSizeMake(HJPageTitleViewWidth, HJPageTitleViewHeight);

    } else { // HJPageTitleView 滚动样式
        CGFloat btnX = 0;
        CGFloat btnY = 0;
        CGFloat btnH = 0;
        if (self.configure.indicatorStyle == HJIndicatorStyleDefault) {
        ...
        self.scrollView.contentSize = CGSizeMake(scrollViewWidth, HJPageTitleViewHeight);
    }
...
- (void)P_btn_action:(UIButton *)button {
    ...
    // 2、标题滚动样式下选中标题居中处理
    if (self.allBtnWidth > HJPageTitleViewWidth) {
        _signBtnClick = YES;
...
- (void)P_selectedBtnCenter:(UIButton *)centerBtn {
    // 计算偏移量
    CGFloat offsetX = centerBtn.center.x - HJPageTitleViewWidth * 0.5;
    if (offsetX < 0) offsetX = 0;
    // 获取最大滚动范围
    CGFloat maxOffsetX = self.scrollView.contentSize.width - HJPageTitleViewWidth;
...
- (void)P_changeIndicatorWithButton:(UIButton *)button {
    [UIView animateWithDuration:self.configure.indicatorAnimationTime animations:^{
        if (self.configure.indicatorStyle == HJIndicatorStyleFixed) {
            self.indicatorView.HJ_width = self.configure.indicatorFixedWidth;
            self.indicatorView.HJ_centerX = button.HJ_centerX;
            return;
        }
        
        if (self.configure.indicatorStyle == HJIndicatorStyleDynamic) {
            self.indicatorView.HJ_width = self.configure.indicatorDynamicWidth;
            self.indicatorView.HJ_centerX = button.HJ_centerX;
            return;
        }
        ...
        if (tempIndicatorWidth > button.HJ_width) {
            tempIndicatorWidth = button.HJ_width;
        }
        self.indicatorView.HJ_width = tempIndicatorWidth;
        self.indicatorView.HJ_centerX = button.HJ_centerX;
    }];
}
...
- (void)setPageTitleViewWithProgress:(CGFloat)progress originalIndex:(NSInteger)originalIndex targetIndex:(NSInteger)targetIndex {
    ...
    // 3、处理指示器的逻辑
    if (self.allBtnWidth <= self.bounds.size.width) { /// HJPageTitleView 静止样式
        if (self.configure.indicatorScrollStyle == HJIndicatorScrollStyleDefault) {
...
- (void)addBadgeForIndex:(NSInteger)index {
    ...
    CGFloat badgeX = 0.5 * (btn.HJ_width - btnTextWidth) + btnTextWidth + self.configure.badgeOff.x;
    CGFloat badgeY = 0.5 * (btn.HJ_height - btnTextHeight) + self.configure.badgeOff.y - self.configure.badgeSize;
...
- (void)resetTitle:(NSString *)title forIndex:(NSInteger)index {
    ...
    if (_signBtnIndex == index) {
        if (self.configure.indicatorStyle == HJIndicatorStyleDefault || self.configure.indicatorStyle == HJIndicatorStyleCover) {
            CGSize tempSize = [self P_sizeWithString:button.currentTitle font:self.configure.titleFont];
            CGFloat tempIndicatorWidth = self.configure.indicatorAdditionalWidth + tempSize.width;
            if (tempIndicatorWidth > button.HJ_width) {
                tempIndicatorWidth = button.HJ_width;
 ...
 */
- (void)setImages:(NSArray *)images selectedImages:(NSArray *)selectedImages imagePositionType:(HJImagePositionType)imagePositionType spacing:(CGFloat)spacing {
    ...
    if (imagesCount < selectedImagesCount) {
        NSLog(@"温馨提示：HJPageTitleView -> [setImages:selectedImages:imagePositionType:spacing] 方法中 images 必须大于或者等于selectedImages，否则 imagePositionTypeDefault 以外的其他样式图片及文字布局将会出现问题");
    }
 ...
 *  @param index        标题对应下标值
 */
- (void)setImage:(NSString *)image selectedImage:(NSString *)selectedImage imagePositionType:(HJImagePositionType)imagePositionType spacing:(CGFloat)spacing forIndex:(NSInteger)index {
...
- (void)P_btn:(UIButton *)btn imageName:(NSString *)imageName imagePositionType:(HJImagePositionType)imagePositionType spacing:(CGFloat)spacing btnControlState:(UIControlState)btnControlState {
    if (imagePositionType == HJImagePositionTypeDefault) {
        [btn HJ_imagePositionStyle:HJImagePositionStyleDefault spacing:spacing imagePositionBlock:^(UIButton *button) {
            [btn setImage:[UIImage imageNamed:imageName] forState:btnControlState];
        }];
        return;
    }
    if (imagePositionType == HJImagePositionTypeRight) {
        [btn HJ_imagePositionStyle:HJImagePositionStyleRight spacing:spacing imagePositionBlock:^(UIButton *button) {
            [btn setImage:[UIImage imageNamed:imageName] forState:btnControlState];
        }];
        return;
    }
    if (imagePositionType == HJImagePositionTypeTop) {
        [btn HJ_imagePositionStyle:HJImagePositionStyleTop spacing:spacing imagePositionBlock:^(UIButton *button) {
            [btn setImage:[UIImage imageNamed:imageName] forState:btnControlState];
        }];
        return;
    }
    if (imagePositionType == HJImagePositionTypeBottom) {
        [btn HJ_imagePositionStyle:HJImagePositionStyleBottom spacing:spacing imagePositionBlock:^(UIButton *button) {
            [btn setImage:[UIImage imageNamed:imageName] forState:btnControlState];
        }];
    }
}

#pragma mark - - - HJPageTitleView 静止样式下指示器默认滚动样式（HJIndicatorScrollStyleDefault）
- (void)P_staticIndicatorScrollStyleDefaultWithProgress:(CGFloat)progress originalBtn:(UIButton *)originalBtn targetBtn:(UIButton *)targetBtn {
    ...
    /// 处理 HJIndicatorStyleFixed 样式
    if (self.configure.indicatorStyle == HJIndicatorStyleFixed) {
        CGFloat btnWidth = self.HJ_width / self.titleArr.count;
        ...
        self.indicatorView.HJ_x = originalBtnIndicatorX + progress * totalOffsetX;
        return;
    }
    
    /// 处理 HJIndicatorStyleDynamic 样式
    if (self.configure.indicatorStyle == HJIndicatorStyleDynamic) {
        ...
        if (originalBtnTag <= targetBtnTag) { // 往左滑
            if (progress <= 0.5) {
                self.indicatorView.HJ_width = self.configure.indicatorDynamicWidth + 2 * progress * btnWidth;
            } else {
                ...
                self.indicatorView.HJ_x = targetBtnIndicatorX + 2 * (progress - 1) * btnWidth;
                self.indicatorView.HJ_width = self.configure.indicatorDynamicWidth + 2 * (1 - progress) * btnWidth;
            }
        } else {
            if (progress <= 0.5) {
                ...
                self.indicatorView.HJ_x = originalBtnIndicatorX - 2 * progress * btnWidth;
                self.indicatorView.HJ_width = self.configure.indicatorDynamicWidth + 2 * progress * btnWidth;
            } else {
                ...
                self.indicatorView.HJ_x = targetBtnIndicatorX; // 这句代码必须写，防止滚动结束之后指示器位置存在偏差，这里的偏差是由于 progress >= 0.8 导致的
                self.indicatorView.HJ_width = self.configure.indicatorDynamicWidth + 2 * (1 - progress) * btnWidth;
            }
        }
        return;
    ...
    self.indicatorView.HJ_x = originalIndicatorX + offsetX;
    
    CGFloat tempIndicatorWidth = self.configure.indicatorAdditionalWidth + originalBtnTextWidth + distance;
    if (tempIndicatorWidth >= targetBtn.HJ_width) {
        CGFloat moveTotalX = targetBtn.HJ_origin.x - originalBtn.HJ_origin.x;
        CGFloat moveX = moveTotalX * progress;
        self.indicatorView.HJ_centerX = originalBtn.HJ_centerX + moveX;
    } else {
        self.indicatorView.HJ_width = tempIndicatorWidth;
    }
}

#pragma mark - - - HJPageTitleView 滚动样式下指示器默认滚动样式（HJIndicatorScrollStyleDefault）
- (void)P_indicatorScrollStyleDefaultWithProgress:(CGFloat)progress originalBtn:(UIButton *)originalBtn targetBtn:(UIButton *)targetBtn {
    ...
    /// 处理 HJIndicatorStyleFixed 样式
    if (self.configure.indicatorStyle == HJIndicatorStyleFixed) {
        CGFloat targetIndicatorX = CGRectGetMaxX(targetBtn.frame) - 0.5 * (targetBtn.HJ_width - self.configure.indicatorFixedWidth) - self.configure.indicatorFixedWidth;
        CGFloat originalIndicatorX = CGRectGetMaxX(originalBtn.frame) - self.configure.indicatorFixedWidth - 0.5 * (originalBtn.HJ_width - self.configure.indicatorFixedWidth);
        CGFloat totalOffsetX = targetIndicatorX - originalIndicatorX;
        CGFloat offsetX = totalOffsetX * progress;
        self.indicatorView.HJ_x = originalIndicatorX + offsetX;
        return;
    ...
    if (self.configure.indicatorStyle == HJIndicatorStyleDynamic) {
        ...
        if (originalBtnTag <= targetBtnTag) { // 往左滑
            ...
            CGFloat btnCenterXDistance = targetBtn.HJ_centerX - originalBtn.HJ_centerX;
            if (progress <= 0.5) {
                self.indicatorView.HJ_width = 2 * progress * btnCenterXDistance + self.configure.indicatorDynamicWidth;
            } else {
                CGFloat targetBtnX = CGRectGetMaxX(targetBtn.frame) - self.configure.indicatorDynamicWidth - 0.5 * (targetBtn.HJ_width - self.configure.indicatorDynamicWidth);
                self.indicatorView.HJ_x = targetBtnX + 2 * (progress - 1) * btnCenterXDistance;
                self.indicatorView.HJ_width = 2 * (1 - progress) * btnCenterXDistance + self.configure.indicatorDynamicWidth;
            }
        } else {
            // originalBtn 与 targetBtn 中心点之间的距离
            CGFloat btnCenterXDistance = originalBtn.HJ_centerX - targetBtn.HJ_centerX;
            if (progress <= 0.5) {
                CGFloat originalBtnX = CGRectGetMaxX(originalBtn.frame) - self.configure.indicatorDynamicWidth - 0.5 * (originalBtn.HJ_width - self.configure.indicatorDynamicWidth);
                self.indicatorView.HJ_x = originalBtnX - 2 * progress * btnCenterXDistance;
                self.indicatorView.HJ_width = 2 * progress * btnCenterXDistance + self.configure.indicatorDynamicWidth;
            } else {
                CGFloat targetBtnX = CGRectGetMaxX(targetBtn.frame) - self.configure.indicatorDynamicWidth - 0.5 * (targetBtn.HJ_width - self.configure.indicatorDynamicWidth);
                self.indicatorView.HJ_x = targetBtnX; // 这句代码必须写，防止滚动结束之后指示器位置存在偏差，这里的偏差是由于 progress >= 0.8 导致的
                self.indicatorView.HJ_width = 2 * (1 - progress) * btnCenterXDistance + self.configure.indicatorDynamicWidth;
            }
        }
        return;
    ...
    CGFloat totalOffsetX = targetBtn.HJ_x - originalBtn.HJ_x;
    ...
    if (tempIndicatorWidth >= targetBtn.HJ_width) {
        ...
        self.indicatorView.HJ_x = originalBtn.HJ_x + offsetX;
        self.indicatorView.HJ_width = originalBtn.HJ_width + distance;
    } else {
        ...
        self.indicatorView.HJ_x = originalBtn.HJ_x + offsetX;
        self.indicatorView.HJ_width = originalBtn.HJ_width + distance + self.configure.indicatorAdditionalWidth;
    }
}

#pragma mark - - - HJPageTitleView 静止样式下指示器 HJIndicatorScrollStyleHalf 和 HJIndicatorScrollStyleEnd 滚动样式
- (void)P_staticIndicatorScrollStyleHalfEndWithProgress:(CGFloat)progress originalBtn:(UIButton *)originalBtn targetBtn:(UIButton *)targetBtn {
    /// 1、处理 HJIndicatorScrollStyleHalf 逻辑
    if (self.configure.indicatorScrollStyle == HJIndicatorScrollStyleHalf) {
        // 1、处理 HJIndicatorStyleFixed 样式
        if (self.configure.indicatorStyle == HJIndicatorStyleFixed) {
            if (progress >= 0.5) {
                [UIView animateWithDuration:self.configure.indicatorAnimationTime animations:^{
                    self.indicatorView.HJ_centerX = targetBtn.HJ_centerX;
                    [self P_changeSelectedButton:targetBtn];
                }];
            } else {
                [UIView animateWithDuration:self.configure.indicatorAnimationTime animations:^{
                    self.indicatorView.HJ_centerX = originalBtn.HJ_centerX;
        ...
        if (progress >= 0.5) {
            ...
            [UIView animateWithDuration:self.configure.indicatorAnimationTime animations:^{
                if (tempIndicatorWidth >= targetBtn.HJ_width) {
                    self.indicatorView.HJ_width = targetBtn.HJ_width;
                } else {
                    self.indicatorView.HJ_width = tempIndicatorWidth;
                }
                self.indicatorView.HJ_centerX = targetBtn.HJ_centerX;
            ...
            [UIView animateWithDuration:self.configure.indicatorAnimationTime animations:^{
                if (tempIndicatorWidth >= targetBtn.HJ_width) {
                    self.indicatorView.HJ_width = originalBtn.HJ_width;
                } else {
                    self.indicatorView.HJ_width = tempIndicatorWidth;
                }
                self.indicatorView.HJ_centerX = originalBtn.HJ_centerX;
    ...
    /// 2、处理 HJIndicatorScrollStyleEnd 逻辑
    // 1、处理 HJIndicatorStyleFixed 样式
    if (self.configure.indicatorStyle == HJIndicatorStyleFixed) {
            if (progress == 1.0) {
                [UIView animateWithDuration:self.configure.indicatorAnimationTime animations:^{
...
#pragma mark - - - HJPageTitleView 滚动样式下指示器 HJIndicatorScrollStyleHalf 和 HJIndicatorScrollStyleEnd 滚动样式
- (void)P_indicatorScrollStyleHalfEndWithProgress:(CGFloat)progress originalBtn:(UIButton *)originalBtn targetBtn:(UIButton *)targetBtn {
    ...
    if (self.configure.indicatorScrollStyle == HJIndicatorScrollStyleHalf) {
        ...
        if (progress >= 0.5) {
            ...
            CGFloat tempIndicatorWidth = self.configure.indicatorAdditionalWidth + tempSize.width;
            [UIView animateWithDuration:self.configure.indicatorAnimationTime animations:^{
                if (tempIndicatorWidth >= originalBtn.HJ_width) {
                    self.indicatorView.HJ_width = originalBtn.HJ_width;
                } else {
                    self.indicatorView.HJ_width = tempIndicatorWidth;
                }
