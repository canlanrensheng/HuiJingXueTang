...
#import "HJPageContentCollectionView.h"
#import "UIView+HJPagingView.h"

@interface HJPageContentCollectionView () <UICollectionViewDataSource, UICollectionViewDelegate>
/// 外界父控制器
...
@implementation HJPageContentCollectionView
...
- (instancetype)initWithFrame:(CGRect)frame parentVC:(UIViewController *)parentVC childVCs:(NSArray *)childVCs {
    if (self = [super initWithFrame:frame]) {
        if (parentVC == nil) {
            @throw [NSException exceptionWithName:@"HJPagingView" reason:@"HJPageContentCollectionView 初始化方法中所在控制器必须设置" userInfo:nil];
        }
        self.parentViewController = parentVC;
        if (childVCs == nil) {
            @throw [NSException exceptionWithName:@"HJPagingView" reason:@"HJPageContentCollectionView 初始化方法中子控制器必须设置" userInfo:nil];
        }
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        CGFloat collectionViewW = self.HJ_width;
        CGFloat collectionViewH = self.HJ_height;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    // 3、pageContentCollectionViewDelegate; 将 progress／sourceIndex／targetIndex 传递给 HJPageTitleView
...
#pragma mark - - - 给外界提供的方法，获取 HJPageTitleView 选中按钮的下标
- (void)setPageContentCollectionViewCurrentIndex:(NSInteger)currentIndex {
    CGFloat offsetX = currentIndex * self.collectionView.HJ_width;
