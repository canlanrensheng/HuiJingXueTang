...
#import "HJPageContentScrollView.h"
#import "UIView+HJPagingView.h"

@interface HJPageContentScrollView () <UIScrollViewDelegate>
// 外界父控制器
...
@implementation HJPageContentScrollView

- (instancetype)initWithFrame:(CGRect)frame parentVC:(UIViewController *)parentVC childVCs:(NSArray *)childVCs {
    if (self = [super initWithFrame:frame]) {
        if (parentVC == nil) {
            @throw [NSException exceptionWithName:@"HJPagingView" reason:@"HJPageContentScrollView 初始化方法中所在控制器必须设置" userInfo:nil];
        }
        self.parentViewController = parentVC;
        if (childVCs == nil) {
            @throw [NSException exceptionWithName:@"HJPagingView" reason:@"HJPageContentScrollView 初始化方法中子控制器必须设置" userInfo:nil];
        }
...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
        ...
        CGFloat contentWidth = self.childViewControllers.count * _scrollView.HJ_width;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (firstAdd) {
        ...
        childVC.view.frame = CGRectMake(offsetX, 0, self.HJ_width, self.HJ_height);
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    // 3、pageContentScrollViewDelegate; 将 progress／sourceIndex／targetIndex 传递给 HJPageTitleView
...
- (void)setPageContentScrollViewCurrentIndex:(NSInteger)currentIndex {
    // 1、根据标题下标计算 pageContent 偏移量
    CGFloat offsetX = currentIndex * self.HJ_width;
