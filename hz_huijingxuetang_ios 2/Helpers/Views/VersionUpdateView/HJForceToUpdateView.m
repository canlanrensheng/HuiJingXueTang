...
@interface HJForceToUpdateView()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>{
...
@implementation HJForceToUpdateView
...
//        self.tableView.backgroundColor = red_color;
        ...
        [nowToUpdateBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.right.equalTo(kuangImagV);
        ...
        [versionNumLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.equalTo(alertView);
            make.bottom.equalTo(nowToUpdateBtn.mas_top).offset(-kHeight(16));
...
- (HJForceToUpdateView * )initWithVersionNum:(NSString *)versionNum versionUpdateContent:(NSString *)versionUpdateContent link:(NSString *)link bindBlock:(void(^)(BOOL success))bindBlock {
    ...
    self.link = link;
    HJForceToUpdateView *alretView = [self initWithFrame:CGRectMake(0, 0, Screen_Width, Screen_Height)];
