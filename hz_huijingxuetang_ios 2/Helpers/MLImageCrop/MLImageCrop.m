...
static const CGFloat kDefualRatioOfWidthAndHeight = 1.0f;
...
- (void)viewDidLoad
{
    ...
    [self.view addSubview:self.confirmButton = [self acquireCustomButtonWithTitle:@"完成" andAction:@selector(onConfirm:)]];
...
//    _pr(cropRect);
    ...
    if (self.outputWidth) {
        CGFloat scale = cropImage.size.height/cropImage.size.width;
        cropImage = [self imageWithImageSimple:cropImage scaledToSize:CGSizeMake(self.outputWidth, self.outputWidth * scale)];
    }
...
- (void)viewDidLayoutSubviews
{
    ...
    //底部按钮背景View
    static const CGFloat kButtonViewHeight = 50;
