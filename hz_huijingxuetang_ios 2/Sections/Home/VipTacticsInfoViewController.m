//
//  VipTacticsInfoViewController.m
//  HuiJingSchool
//
//  Created by Junier on 2018/2/28.
...
#import "GiftView.h"
#import "FSTextView.h"
@interface VipTacticsInfoViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
...
@implementation VipTacticsInfoViewController
{
    UIButton *menbanview;
    UIView *giftview;
...
-(void)loadInfo{
    [YJAPPNetwork VIPtacticsInfowithaccesstoken:[APPUserDataIofo AccessToken] infoid:_Id success:^(NSDictionary *responseObject) {
...
-(void)getmainview{
    ...
    toplabel.text = [datadic objectForKey:@"articaltitle"];
    ...
    toplabel1.text = [NSString stringWithFormat:@"%@  %@   阅读：%ld",[datadic objectForKey:@"realname"],[datadic objectForKey:@"tdays"],[[datadic objectForKey:@"readcount"]integerValue]];
    toplabel1.textColor = TextColor;
    toplabel1.font = TextsmlFont;
    [self.view addSubview:toplabel1];
    
    UIImageView *imgview = [[UIImageView alloc]initWithFrame:CGRectMake(15*SW, toplabel1.maxY+35*SW, kW - 30*SW, 300*SW)];
    imgview.backgroundColor = ALLViewBgColor;
    [self.view addSubview:imgview];
    
    
    UILabel *titleview = [[UILabel alloc]initWithFrame:CGRectMake(0, imgview.maxY+15*SW, kW, 35*SW)];
    ...
    [self.view addSubview:ln1view];
    
    
    _tableview1 = [[UITableView alloc]initWithFrame:CGRectMake(0, titleview.maxY, kW,self.view.height -titleview.maxY-45)];
...
-(void)huifuAction:(UIButton *)sender{
    NSInteger index = sender.tag - 1567;

    NSDictionary *dic = commentarr[index];

    
    menbanview = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, kW,self.view.height)];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSURL *url = [dic objectForKey:@"iconurl"];
