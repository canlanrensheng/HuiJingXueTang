...
@implementation HJInviteCodeView{
...
- (void)creatView{
    ...
    CGFloat lineHeight = kHeight(2.0);
    ...
    for (int i = 0 ; i < _num; i++) {
        UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(i * width + i *((self.frame.size.width - _num * width)/(_num - 1)), self.frame.size.height - lineHeight, width, lineHeight)];
    ...
    for (int i =0 ; i<_num; i++) {
        ...
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(i * width+i*((self.frame.size.width - _num * width)/(_num-1)),0, width, self.frame.size.height - lineHeight)];
        label.textAlignment = NSTextAlignmentCenter;
        label.font = self.labelFont;
        label.textColor = self.textLabelColor;
        [self addSubview:label];
        
        UIBezierPath *path = [UIBezierPath bezierPathWithRect:CGRectMake((CGRectGetWidth(label.frame) - kWidth(1.0)) /2,0,kWidth(1.0),(CGRectGetHeight(label.frame) - kHeight(11.0)))];
...
- (void)showPassword{
    //设置属性默认颜色
    self.lineColor = HEXColor(@"#22476B");
    self.lineViewColor = HEXColor(@"#22476B");
    self.selectlineViewColor = HEXColor(@"#22476B");
    self.errorlineViewColor = [UIColor redColor];
    self.labelFont = MediumFont(font(25));
    self.textLabelColor = [UIColor colorWithHexString:@"#333333"];
