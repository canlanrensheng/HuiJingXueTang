...
@interface HJMyShareCommunityCell ()

@property (nonatomic,strong) UILabel *monthLabel;
@property (nonatomic,strong) UILabel *incomeLabel;
@property (nonatomic,strong) UILabel *finishOrderLabel;
@property (nonatomic,strong) UILabel *finishDateLabel;

@end

@implementation HJMyShareCommunityCell
...
- (void)hj_configSubViews {
    ...
    NSString *dateString = @"03期/12月";
    monthLabel.attributedText = [dateString attributeWithStr:@"/12月" color:HEXColor(@"#22476B") font:BoldFont(font(10.0))];
    ...
    UILabel *incomeLabel = [UILabel creatLabel:^(UILabel *label) {
        label.ljTitle_font_textColor(@"社区人数  0",MediumFont(font(13)),HEXColor(@"#666666"));
        label.textAlignment = NSTextAlignmentLeft;
        label.numberOfLines = 0;
        [label sizeToFit];
    }];
    ...
    [incomeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(monthLabel.mas_bottom).offset(kHeight(12.0));
        make.left.equalTo(topView).offset(kWidth(10.0));
    ...
    UILabel *freezeMoneyLabel = [UILabel creatLabel:^(UILabel *label) {
        label.ljTitle_font_textColor(@"成交订单  0",MediumFont(font(13)),HEXColor(@"#666666"));
        label.textAlignment = NSTextAlignmentLeft;
        label.numberOfLines = 0;
        [label sizeToFit];
    }];
    ...
    self.finishOrderLabel = freezeMoneyLabel;
...
- (void)setViewModel:(BaseViewModel *)viewModel indexPath:(NSIndexPath *)indexPath {
    HJMyShareCommunityViewModel *listViewModel = (HJMyShareCommunityViewModel *)viewModel;
    HJPromoteInfoModel *model = listViewModel.promoteInfoArray[indexPath.row];
    if(model.datadaterange) {
//        NSString *year = [model.datadaterange substringToIndex:4];
//        NSString *month = [model.datadaterange substringFromIndex:4];
//        self.monthLabel.text = [NSString stringWithFormat:@"%@年%@月",year,month];
    ...
    NSString *commutityNum = [NSString stringWithFormat:@"社区人数  %ld",model.commcount];
    self.incomeLabel.attributedText = [commutityNum attributeWithStr:[NSString stringWithFormat:@"%ld",model.commcount] color:HEXColor(@"#FF4400") font:BoldFont(font(13))];
    
    NSString *finishOrder = [NSString stringWithFormat:@"成交订单  %ld",model.ordercount];
    self.finishOrderLabel.attributedText = [finishOrder attributeWithStr:[NSString stringWithFormat:@"%ld",model.ordercount] color:HEXColor(@"#FF4400") font:BoldFont(font(13))];
